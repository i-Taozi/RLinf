...
def multi_call_with_timeout(funcs, *args, timeout=5, **kwargs):
    def worker(func, args, kwargs, result_list, i):
        try:
            result_list[i] = func(*args, **kwargs)
        except:
            pass

    manager = multiprocessing.Manager()
    result_list = manager.list([None] * len(funcs))
    jobs: list[multiprocessing.Process] = []
    for i, func in enumerate(funcs):
        p = multiprocessing.Process(target=worker, args=(func, args, kwargs, result_list, i))
        jobs.append(p)
        p.start()

    timeout_to = time.time() + timeout
    for i, proc in enumerate(jobs):
        proc.join(max(0, timeout_to - time.time()))
        if proc.is_alive():
            print(f'multi_call_with_timeout catch timeout, at func{funcs[i]}, args={args}, kwargs={kwargs}')
            proc.terminate()
            proc.join()

    return [i for i in result_list]
...
def symbolic_equal(a, b):
    ...
    return_list = multi_call_with_timeout([direct_equal, simplify_equal, equation_equal, numeric_equal, matrix_equal], a, b, timeout=5)
    return any(return_list)


def direct_equal(a, b):
    # direct equal
    try:
